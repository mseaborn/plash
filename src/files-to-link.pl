...
my @rtld_files =
 ('csu/rtld-check_fds.os',
  ...
  # llseek only needed for my patch to ld.so
#  'misc/rtld-llseek.os',
  'posix/rtld-_exit.os',
  ...
  # New requirement in 2.3.5:
  'posix/rtld-uname.os',

  # These are not normally used in glibc; I added them myself.
...
foreach my $d (@{dir_leaves($glibc_dir)}) {
...
if(!$rtld) {
  foreach my $f (@dirs) {
    foreach my $x (split(/\s+/, slurp("$glibc_dir/$f/stamp.os"))) {
      push(@files, $x);
    }
  }
}
...
foreach my $f (keys(%$excl)) {
  if($excl->{$f} <= 1) {
    if(!$rtld) {
      print STDERR "Did not exclude $f\n"
    }
  }
  else {
